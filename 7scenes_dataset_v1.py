...
class ScanNetPreprocessor:
    ...
    def process_scene(self, scene_dir):
        ...
        scene_parts = os.path.normpath(scene_dir).split(os.sep)
        scene_id = '/'.join(scene_parts[-2:])
...
class BufferedSceneDataset(Dataset):
    def __init__(self, scene_dirs, max_scenes=10, num_workers=8, num_frames=32, frame_skip=1, 
        ...
        # Scene management
        self.all_scene_dirs = np.array(scene_dirs)
...
# Example usage
def main():
    # Helper to parse train scenes from split_7scenes.txt
    def get_train_scenes(split_file):
        ...
        for line in lines:
            line = line.strip()
            if line.startswith('Train scenes:'):
                in_train = True
                continue
            if line.startswith('Test scenes:'):
                break
            if in_train and line:
                train_scenes.append(line)
        return train_scenes

    data_transforms = None
    train_scene_list = get_train_scenes('split_7scenes.txt')
    buffer_scene = BufferedSceneDataset(
        scene_dirs=train_scene_list,
        max_scenes=10,
        num_workers=8,
        num_frames=16,
        frame_skip=1,
    ...
    print(f'Number of train scenes is: {len(train_scene_list)}')
