...
def rescale_image_depthmap_preddepth(image, depthmap, pred_depth, camera_intrinsics, output_resolution, force=True):
    """ Jointly rescale a (image, depthmap) 
        so that (out_width, out_height) >= output_res
    """
    image = ImageList(image)
    ...
    return image.to_pil(), pred_depth, depthmap, camera_intrinsics
