...
class SimplerPoseHead(nn.Module):
    ...
    def axis_angle_to_matrix(self, vec):
        ...
        Returns:
            rot_mat: Tensor of shape (N, 3, 3)
        """
        theta = torch.norm(vec, dim=1, keepdim=True) + 1e-8
        axis = vec / theta

        a = torch.cos(theta / 2)
        b, c, d = -axis[:, 0] * torch.sin(theta / 2), -axis[:, 1] * torch.sin(theta / 2), -axis[:, 2] * torch.sin(theta / 2)
...
if __name__ == "__main__":
    ...
    final_pose = pose_head.forward(fused_tokens)
