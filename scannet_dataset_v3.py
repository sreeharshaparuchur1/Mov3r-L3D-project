...
class ScanNetPreprocessor:
    ...
    def load_dataset(self, save_to_disk=True, return_data=False):
        ...
        for scene_data in all_scenes_data:
            ...
            rgb = torch.from_numpy(rgb).permute(0,3,1,2).to(torch.float16, non_blocking=True)
            ...
            if not self.rgb_only:
                memory_dataset[scene_id]['depth'] = torch.from_numpy(np.stack(scene_data['depth']))
                memory_dataset[scene_id]['pose'] = torch.from_numpy(np.stack(scene_data['pose']))
...
class ScanNetMemoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        intrinsics = {}
        for k, v in scene_data['intrinsics'].items():
            intrinsics[k] = torch.from_numpy(v)
