...
from scannet_dataset_v3 import ScanNetPreprocessor, ScanNetMemoryDataset
...
def get_config():
    ...
    parser.add_argument('--dino_encoder', type=str, default='vitl', help='DINO encoder type: vitl, vits, vitb, vitg')
...
class Mov3r:
    def __init__(self, args, dist, local_rank):
        ...
        self.dino_patch_embed= DINOEmbedder(patch_embed='dinov2_vitl14_reg', img_size=224, patch_size=16)
        ...
        self.point_head = DPTHead(dim_in=2 * args.embed_dim, output_dim=4, activation="inv_log", conf_activation="expp1")
        self.depth_head = DPTHead(dim_in=2 * args.embed_dim, output_dim=2, activation="exp", conf_activation="expp1")

        if torch.cuda.device_count() > 1:
            #data parallel for ViT Model selfAttention
            self.depth_embedder = self.depth_embedder.to(f'cuda:{local_rank}')
            self.depth_embedder = torch.nn.parallel.DistributedDataParallel(self.depth_embedder, device_ids=[local_rank])
            
            self.dino_patch_embed =  self.dino_patch_embed.to(f'cuda:{local_rank}')
            ...
            self.dust3r_cross = self.dust3r_cross.to(f'cuda:{local_rank}')
            ...
            self.point_head = self.point_head.to(f'cuda:{local_rank}')
            self.point_head = torch.nn.parallel.DistributedDataParallel(self.point_head, device_ids=[local_rank])

            self.depth_head = self.depth_head.to(f'cuda:{local_rank}')
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_idx, batch in enumerate(epoch_bar):
                rgb, depth, pose, intrinsic = (batch["rgb"].to(device, non_blocking=True), batch["depth"].to(device, non_blocking=True), batch["pose"].to(device, non_blocking=True), batch
                ["intrinsics"])
                B, S, C, H, W = rgb.shape
                ...
                intrinsic_depth = intrinsic['intrinsic_depth'].to(device, non_blocking=True)
                ...
                print(intrinsic['intrinsic_depth'])
